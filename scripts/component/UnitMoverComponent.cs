...
public partial class UnitMoverComponent : Node, IController
{
    ...
    private int GetPlayAreaForPosition(Vector2 global)
    {
        ...
        for (var i = 0; i < PlayerAreas!.Count; i++)
        {
            ...
            if (playerArea.IsTileInBounds(tile)) index = i;
        }
    ...
    public override void _Ready()
    {
        ...
        foreach (var unit in units) SetupUnit(unit as Unit);
    }
    ...
    private void SetHighlighters(bool enable)
    {
        ...
        foreach (var playerAreaComponent in PlayerAreas) playerAreaComponent.TileHighlighter!.Enable = enable;
    }
    ...
    public void SetupUnit(Unit? unit)
    {
        unit!.DragDropComponent!.Connect(DragDropComponent.SignalName.DragStarted,
            Callable.From(() => OnDragStarted(unit)));
